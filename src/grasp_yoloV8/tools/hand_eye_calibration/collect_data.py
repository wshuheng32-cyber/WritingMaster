...
def callback(frame):
    ...
    if k != 255:
        logger_.info(f"检测到按键，keycode={k}")

    if k in (ord('s'), ord('S')):  # 若检测到按键 's' 或 'S'，保存当前图像和机械臂位姿
...
def displayD435():
    ...
    logger_.info("请先点击相机窗口，再按 S 采集，按 Q 或 ESC 退出")
    ...
    try:
        ...
        while True:
            try:
                ...
                logger_.warning(f"等待相机帧超时，第 {consecutive_timeouts} 次：{e}")

                if consecutive_timeouts >= 3:
                    popup_message("提醒", "连续3次未获取到D435图像，请检查相机连接、供电或是否被其他程序占用")
                    logger_.error_("连续3次未获取到D435图像，请检查相机连接、供电或是否被其他程序占用")
                    sys.exit(1)
