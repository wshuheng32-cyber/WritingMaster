...
/*****************************设置寄存器返回话题**************************/
ros::Publisher pub_WriteRegister_result;
/*****************************设置modbus模式返回话题**************************/
ros::Publisher pub_SetModbusMode_result;
/*****************************设置工具端数字IO输出状态**************************/
...
#define WRITE_SINGLE_REGISTER 0x37
#define SET_MODBUS_MODE 0x38
#define WRITE_REGISTER 0x39
...
//写寄存器
int Write_Register_Cmd(int port, int address, int num,long long *wdata, int device)
{
    cJSON *root, *array;
    char *data;
    char buffer[200];
    ...
    cJSON_AddNumberToObject(array, "test", wdata[0]);
    cJSON_AddNumberToObject(array, "test", wdata[1]);
...
//设置modbus模式
int Set_Modbus_Mode_Cmd(int port, int baudrate, int timeout)
{
    cJSON *root;
    char *data;
    char buffer[200];
...
int Parser_Write_Register_Cmd(char *msg)
{
    cJSON *root = NULL, *result;
    root = cJSON_Parse(msg);
    ...
    {
        RM_Joint.state = result->valueint;
        ROS_INFO("RM_Joint.Write_Register: %d", RM_Joint.state);
        cJSON_Delete(root);
        return 0;
    }
    else
...
int Parser_Set_Set_Modbus_Mode_Cmd(char *msg)
{
    cJSON *root = NULL, *result;
    root = cJSON_Parse(msg);
    ...
    if (json_state != NULL)
    {
        if (!strcmp("start_multi_drag_teach", json_state->valuestring))
        {
        ...
        else if (!strcmp("set_modbus_mode", json_state->valuestring))
            {
                ...
                if (json_state != NULL)
                {
                    res = Parser_Set_Set_Modbus_Mode_Cmd(msg);
                    if (res == 0)
                    {
                        cJSON_Delete(root);
        ...
        else if (!strcmp("write_register", json_state->valuestring))
            {
                json_state = cJSON_GetObjectItem(root, "write_state");
                if (json_state != NULL)
                {
                    res = Parser_Write_Register_Cmd(msg);
                    if (res == 0)
                    {
                        cJSON_Delete(root);
