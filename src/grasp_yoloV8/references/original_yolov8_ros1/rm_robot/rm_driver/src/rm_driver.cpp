...
// 设置modbus模式回调函数
void Set_Modbus_Mode_Cmd_Callback(const rm_msgs::set_modbus_mode msg)
{
    int res = 0;
    int port = msg.port;
    int baudrate = msg.baudrate;
    int timeout = msg.timeout;

    res = Set_Modbus_Mode_Cmd(port,baudrate,timeout);
    if (res != 0)
    {
        ROS_ERROR("Set_Modbus_Mode_Cmd failed!\n");
    }
    else
    {
        ROS_INFO("Set_Modbus_Mode_Cmd send success!\n");
    }
}

// 写寄存器回调函数
void Write_Register_Cmd_Callback(const rm_msgs::write_register msg)
{
    int res = 0;
    long long wdata[2] ;
    int port = msg.port;
    ...
    res = Write_Register_Cmd(port,address,num,wdata, device);
    if (res != 0)
    {
        ROS_ERROR("Write_Register_Cmd failed!\n");
    }
    else
    {
        ROS_INFO("Write_Register_Cmd send success!\n");
    }
}
...
void Write_Single_Register_Cmd_Callback(const rm_msgs::write_single_register msg)
{
    ...
    res = Write_Single_Register_Cmd(port,address,data, device);
    if (res != 0)
    {
...
int main(int argc, char **argv)
{
    ...
    sub_WriteRegister = nh_.subscribe("/rm_driver/Write_Register_Cmd", 10, Write_Register_Cmd_Callback);
    sub_SetModbusMode = nh_.subscribe("/rm_driver/Set_Modbus_Mode_Cmd", 10, Set_Modbus_Mode_Cmd_Callback);
    ...
    /**************************************写寄存器返回***************************************/
    pub_WriteRegister_result = nh_.advertise<std_msgs::Bool>("/rm_driver/Write_Register_result", 1);
    /**************************************写单个寄存器返回***************************************/
    pub_WriteSingleRegister_result = nh_.advertise<std_msgs::Bool>("/rm_driver/Write_Single_Register_result", 1);
    /**************************************写modbus模式返回***************************************/
    pub_SetModbusMode_result = nh_.advertise<std_msgs::Bool>("/rm_driver/Set_Modbus_Mode_result", 1);
    /**************************************工具端数字IO输出状态***************************************/
            ...
            if ((temp[0] == 0x0A) && (buffer_cnt > 1))
            {
                ...
                if (msg == 0x0D)
                {
                    ...
                    case WRITE_SINGLE_REGISTER:
                        state.data = RM_Joint.state;
                        pub_WriteSingleRegister_result.publish(state);
                        break;
                    case WRITE_REGISTER:
                        state.data = RM_Joint.state;
                        pub_WriteRegister_result.publish(state);
                        break;
                    case SET_MODBUS_MODE:
                        state.data = RM_Joint.state;
                        pub_SetModbusMode_result.publish(state);
                        break;
